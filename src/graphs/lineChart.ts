...
const lineChart = (data: any) => {
  ...
  const xS = d3.scaleLinear([0, data.length - 1], [0, graphDimensions.width]);
  const yS = d3.scaleLinear([0, 100], [graphDimensions.height, 0]);
  const xSA = d3.axisBottom(xS);
  const ySA = d3.axisLeft(yS);
  const line: any = d3.line((_, i) => xS(i), yS);
  ...
  frame
    ...
    .call(xSA);
  frame.append("g").call(ySA.ticks(10));
  frame
    .append("g")
    .append("path")
    .attr("fill", "none")
